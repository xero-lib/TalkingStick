...
} from "../../coagulators/configCoagulator.js";
...
export default async function (oldMessage, message) {
  if (message.guild) {
    ...
    if (message.content.startsWith(prefix)) {
      console.log(
        date(),
        `Original message from ${oldMessage.author.username}#${oldMessage.author.discriminator} (${oldMessage.author.id}):\n\t`,
        oldMessage.content
      );
      ...
      if (command[0] in CommandMap) {
        ...
        message
          .tempReply(`**\`${prefix}${command}\` is an invalid command.**`)
          .then(() =>
            console.log(
              date(),
    ...
    if (message.content.startsWith(prefix)) {
      ...
      developer.send(dmEmbed).then((s) => {
        console.log(
          date(),
          `${s.username}#${s.discriminator} (${s.id}) updated DM to Talking Stick`
        );
      });
    }
  }
