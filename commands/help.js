...
export default async function (message, args) {
  ...
  if (args) {
    ...
    if (contains == 0) {
      message
        ...
        .then((s) => {
          console.log(
            date(),
            `Successfully sent invalid command reply to ${message.author.username}#${message.author.discriminator} (${message.author.id}) in ${message.channel.name} in server "${message.guild.name}".`
          );
        })
        .catch((e) => {
          message.author
            ...
            .then((s) => {
              console.log(
                date(),
                `Sent invalid command response to  ${chalk.yellow(
            ...
            .catch((e1) => {
              console.log(
                date(),
                `\nCannot send invalid command response to ${chalk.yellow(
                  message.author.username
                )}#${chalk.yellow(message.author.discriminator)} (${
                  message.author.id
    ...
    if (contains != 0) {
      ...
      message.author
        ...
        .then((s) => {
          message
            .tempSend(
              `A help page for \`${helpCmd}\` has been sent to ${message.author.username}`
            )
            .then((s) =>
              console.log(
                date(),
                `${chalk.green("help")} has been sent to ${
                  message.author.username
                }#${message.author.discriminator} (${message.author.id}) in ${
        ...
        .catch((e) => {
          ...
          message.tempSend(
            `Could not send DM to ${message.author.username}, sending temporary help message here.`,
            helpEmbed
          );
        });
    }
    ...
    message
      .tempSend(genHelpEmbed)
      .then((s) =>
        console.log(
          date(),
          `Successfully sent genHelpEmbed in ${message.channel.name} in ${message.guild.name} in response to ${message.author.username}#${message.author.discriminator} (${message.author.id})`
        )
      );
  }
}
