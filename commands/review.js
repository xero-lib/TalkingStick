...
export default async function (interaction) {
    if (interaction.user.id == developer.id) {
        ...
        serverOwners.forEach((e) => e.send(reviewEmbed.setAuthor({ name: botName, iconURL: botPfp })).then(() => console.log(date(),`Sent review invitation to ${e.tag}`)).catch((err) => datedErr(`Could not send review message:`, err)));
    }
}
