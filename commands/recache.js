...
import { developer, client, roles } from '../coagulators/configCoagulator.js'; //see if a server has the ts roles
...
export default async function (message) {
    if(message.author.id == developer.id){
        ...
        await fs.mkdir(path.join(tscacheDir), { recursive: true })
            ...
            .catch(e => console.log(date,`Could not create directory at ${tscacheDir}:`,e))

        const file = `${path.join(tscacheDir,`guildsCache${new Date().toISOString()}.txt`)}`;
        
        console.log('\n',date,`${chalk.redBright.underline.red('====================')}\n${chalk.redBright('User Cache')}`);
        ...
        for (const g of client?.guilds?.cache?.values()) {
            await g.members.fetch()
                .then((s) => {
                    console.log(date,chalk.greenBright(`"${g.name}" has been recached`));
                })
                .catch((e) => {
                    console.log(date,chalk.redBright(`Error recaching "${g.name}"\n`) + chalk.redBright(e))
                });
        }
    }
    else {
        message.tempReply('You do not have permission to execute this command. This incident has been reported.');
        console.log(date,chalk.redBright(`${message.author.username}#${message.author.discriminator} (${message.author.id}) attempted to execute RECACHE without permission.`));
