...
import { findRole, date } from "../barrels/functionCoagulator.js.js";
import {
  defaultPrefix,
  developer,
  Discord,
    ...
    if (findRole(message.guild, "Stick Controller")) {
      if (message.mentions.members.first()) {
        ...
        message.mentions.members
          .first()
          .roles.add(findRole(message.guild, "Stick Controller"))
          .then((s) => {
            tsgiveconEmbed.addField(
              "TSGiveCon:",
              `${
                message.mentions.members.first().username
              } has been given Stick Controller`
            );
        ...
        message.tempReply(
          "**You must mention someone to give Stick Controller.**"
        );
        return;
      }
    } else {
      message.tempReply(
        `Please run \`${defaultPrefix}tsinit\` to create all required roles.`
      );
      return;
    }
  } else {
