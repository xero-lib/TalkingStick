...
export default async function (roleName, message, roleColor) {
    try{
        ...
        console.log(date,`Created ${roleName} in ${message.guild} (${message.guild.id})`);
    }
    catch (err) {
        console.error(date,`Error in ${message.guild} (${message.guild.id}), Create Role command failed. Command issued by ${message.member.displayName} (${message.member.id}):`,err);
